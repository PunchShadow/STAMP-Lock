...
# Progress bar
if specific_bench != "all":
    tqdm_times = times*(math.log(max_thread_num, 2)+1)
else:
    tqdm_times = times*len(cmds_index)*(math.log(max_thread_num, 2)+1)
        ...
        for i in range(times):
            for bench in dic:
                ...
                if bench == 'kmeans' or bench == 'vacation':
                    ...
                    if (specific_bench == (index + '_high')):
                        execute_and_count(high_cmd, index + '_high')
                        pbar.update(1)
                    elif (specific_bench == (index + '_low')):
                        execute_and_count(low_cmd, index + '_low')
                        pbar.update(1)
                    else: 
                        # Execute both high and low command if no specific benchmark
                        execute_and_count(high_cmd, index + '_high')
                        pbar.update(1)
                        execute_and_count(low_cmd, index + '_low')
                        pbar.update(1)
